...
trait SupportTicketManager
{
    ...
    public function viewTicket($ticket)
    {
        ...
        $messages = SupportMessage::where('support_ticket_id', $myTicket->id)->with('ticket', 'admin', 'employee', 'attachments')->orderBy('id', 'desc')->get();
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        if (($this->userType == 'employee') && ($userId != $ticket->agent_id)) {
            abort(404);
        }
        $message = new SupportMessage();
        ...
        $ticket->status = $this->userType == 'user' ? 2 : 1;
        $ticket->last_reply = Carbon::now();
        $ticket->save();
        $message->support_ticket_id = $ticket->id;
        ...
        if ($this->userType == 'employee' && $ticket->user_id != 0) {
