...
class AutomaticGatewayController extends Controller
{
    ...
    public function edit($alias)
    {
        ...
        $supportedCurrencies = $this->normalizeSupportedCurrencies($gateway);
        $parameters = collect(json_decode($gateway->gateway_parameters));

        if ($gateway->alias === 'Fawaterk' && !$parameters->has('provider_key')) {
            $parameters->put('provider_key', (object) [
                'title' => 'Provider Key',
                'global' => true,
                'value' => '',
            ]);
    ...
    public function update(Request $request, $code)
    {
        ...
        if ($gateway->alias === 'Fawaterk' && !$parameters->has('provider_key')) {
            $parameters->put('provider_key', (object) [
                'title' => 'Provider Key',
                'global' => true,
                'value' => $request->global['provider_key'] ?? '',
            ]);
        }
        ...
        $notify[] = ['success', __(':name has been updated successfully', ['name' => $gateway->name])];
    ...
    public function activate($code)
    {
        ...
        $gateway->status = 1;
        $gateway->save();
        $notify[] = ['success', __(':name enabled successfully', ['name' => $gateway->name])];
        return back()->withNotify($notify);
    }
    ...
    public function deactivate($code)
    {
        ...
        $gateway->status = 0;
        $gateway->save();
        $notify[] = ['success', __(':name disabled successfully', ['name' => $gateway->name])];
        return back()->withNotify($notify);
    }
    ...
    public function gatewayCurrencyValidator(Request $request, Gateway $gateway)
    {
        ...
        $supportedCurrencies = $this->normalizeSupportedCurrencies($gateway)->keys()->implode(',');
    ...
    private function normalizeSupportedCurrencies(Gateway $gateway)
    {
        ...
        } elseif (is_object($supportedCurrencies)) {
            $supportedCurrencies = (array) $supportedCurrencies;
        } elseif (! is_array($supportedCurrencies)) {
            $supportedCurrencies = [];
        }

        return collect($supportedCurrencies)->mapWithKeys(function ($symbol, $currency) {
