...
class PageBuilderController extends Controller
{
    ...
    public function managePagesSave(Request $request){
        ...
        $page->save();
        $notify[] = ['success', __('New page has been added successfully')];
        return back()->withNotify($notify);
    ...
    public function managePagesUpdate(Request $request){
        ...
        $notify[] = ['success', __('Page has been updated successfully')];
        return back()->withNotify($notify);
    ...
    public function managePagesDelete($id){
        ...
        $page->delete();
        $notify[] = ['success', __('Page has been deleted successfully')];
        return back()->withNotify($notify);
    }
    ...
    public function manageSection($id)
    {
        ...
        if ($pdata->slug === 'about') {
            ...
            if ($pdata->secs !== json_encode($aboutSections)) {
                $pdata->secs = json_encode($aboutSections);
                $pdata->save();
            }
        }

        $pageTitle = __('Manage :name Page', ['name' => $pdata->name]);
    ...
    public function manageSectionUpdate($id, Request $request)
    {
        ...
        $page->save();
        $notify[] = ['success', __('Page sections has been updated successfully')];
        return back()->withNotify($notify);
    }
}
