...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function showRegistrationForm()
    {
        ...
        $mobileCode = (isset($info['code']) && is_array($info['code'])) ? implode(',', $info['code']) : @$info['code'];
        $countries = json_decode(file_get_contents(resource_path('views/includes/country.json')));
        return view($this->activeTemplate . 'employee.auth.register', compact('pageTitle','mobileCode','countries'));
    }
    ...
    public function register(Request $request)
    {
        ...
        $exist = Employee::where('mobile',$request->mobile_code.$request->mobile)->whereNotNull('mobile')->first();
        if ($request->mobile && $exist) {
    ...
    protected function create(array $data)
    {
        ...
        if ($referBy) {
            $referUser = Employee::where('username', $referBy)->first();
        } else {
            $referUser = null;
        }
        //User Create
        ...
        $adminNotification->click_url = urlPath('admin.employees.detail',$user->id);
        $adminNotification->save();


        //Login Log Create
        $ip = getRealIP();
        $exist = UserLogin::where('user_ip',$ip)->where('agent_id',$user->id)->first();
    ...
    public function checkUser(Request $request){
        ...
        $exist['type'] = null;
        if ($request->email) {
            $exist['data'] = Employee::where('email',$request->email)->exists();
            $exist['type'] = 'email';
        }
        if ($request->mobile) {
            $exist['data'] = Employee::where('mobile',$request->mobile)->exists();
            $exist['type'] = 'mobile';
        }
        if ($request->username) {
            $exist['data'] = Employee::where('username',$request->username)->exists();
            $exist['type'] = 'username';
        }
