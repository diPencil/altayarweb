@extends($activeTemplate . 'layouts.employee.' . $layout)
@section('content')
        ...
        <div class="col-lg-8">
            <div class="base--card">
                <div class="mb-3 d-flex flex-wrap justify-content-between align-items-center">
                    ...
                    @if ($myTicket->status != 3 && $myTicket->user)
                        <button class="btn btn-danger close-button btn-sm confirmationBtn" type="button"
                            ...
                            data-action="{{ route('employee.ticket.close', $myTicket->id) }}"><i
                ...
                @if ($myTicket->status != 4)
                    <form method="post" action="{{ route('employee.ticket.reply', $myTicket->id) }}"
                    ...
                    @foreach ($messages as $message)
                                        ...
                                        @if ($message->attachments->count() > 0)
                                            <div class="mt-2">
                                                @foreach ($message->attachments as $k => $image)
                                                    <a href="{{ route('employee.ticket.download', encrypt($image->id)) }}"
                        ...
                        @elseif ($message->agent_id != 0)
                            <div class="ticket--replay radius--20 mb-3">
                                <div class="row">
                                    <div class="col-md-2 border-end text-start">
                                        <h5 class="my-3">{{ $message->employee?->fullname ?? __('Employee') }}</h5>
