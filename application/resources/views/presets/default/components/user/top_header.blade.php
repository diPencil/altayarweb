@php
  $user = auth()->user();
  $defaultUserImage = asset('assets/images/user/profile/demouser/33/67d132515268a1741763153.png');
  $userImage = $user->image ? getImage(getFilePath('userProfile') . '/' . $user->image, getFileSize('userProfile')) : $defaultUserImage;
...
<div class="row mb-4 mx-0">
    <div class="dashboard-header d-flex justify-content-between align-items-center radius--20">
        ...
        <div class="user-info--wrap d-flex align-items-center gap-3">
            ...
            <a title="@lang('Return to Admin')" href="{{ route('admin.users.stop.impersonate') }}" class="btn btn-sm btn--danger text-white">
                <i class="fas fa-user-shield"></i> <span class="d-none d-md-inline">@lang('Return to Admin')</span><span class="d-inline d-md-none">@lang('Admin')</span>
            </a>
            @endif
            ...
            <a href="javascript:void(0)" class="u-info dropdown-toggle d-flex align-items-center gap-2" data-bs-toggle="dropdown" aria-expanded="false">
                <div class="user-thumb">
                    <img src="{{ $userImage }}" alt="@lang('image')">
                </div>
