    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body ">
                ...
                <form action="{{ route('admin.ticket.assign', $ticket->id) }}" method="post" class="row align-items-end mb-4">
                    @csrf
                    <div class="col-md-8 col-lg-6">
                        <label class="form-label">@lang('Assigned Employee')</label>
                        <select name="agent_id" class="form-control">
                            <option value="">@lang('Unassigned')</option>
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" @selected($ticket->agent_id == $employee->id)>
                                    {{ $employee->fullname }} @if($employee->username) ({{ $employee->username }}) @endif
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 col-lg-3 mt-3 mt-md-0">
                        <button class="btn btn--primary w-100" type="submit">@lang('Assign')</button>
                    </div>
                </form>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                @if($message->admin_id == 0 && $message->agent_id == 0)
                ...
                @elseif($message->agent_id != 0)
                <div class="row admin-reply">
                    <div class="col">
                        <p class="text-muted fw-bold my-3">
                            <span class="text--primary">{{ $message->employee?->fullname ?? __('Employee') }}</span> @lang('replied on')
