                ...
                <div class="card-body p-0">
                    <div class="table-responsive--sm table-responsive admin-table-responsive">
                        <table class="table table--light style--two custom-data-table">
                            ...
                            <tbody>
                                @foreach($general->agent_socialite_credentials as $key => $credential)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end flex-wrap gap-1">
                                                ...
                                                @if ($credential->status == 1)
                                                    <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to enable this login credential?')" data-action="{{ route('admin.setting.socialite.agent.credentials.status.update', $key) }}" title="Disabled"><i class="las la-ban"></i></button>
                                                @else
                                                    <button class="btn btn-outline--success btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to disable login credential?')" data-action="{{ route('admin.setting.socialite.agent.credentials.status.update', $key) }}" title="Enabled"><i class="las la-check"></i></button>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        (function($) {
            ...
            $(document).on('click', '.editBtn', function() {
                let modal = $('#editModal');
                let data = $(this).data();
                let route = "{{ route('admin.setting.socialite.agent.credentials.update', '') }}";
                let callbackUrl = "{{ route('agent.social.login.callback', '') }}";
