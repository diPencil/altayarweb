...
@section('panel')
    ...
    @php $languages = App\Models\Language::where('code', '!=', 'es')->orderBy('is_default','desc')->get(); @endphp
    <div class="row">
                        ...
                        <div class="row">
                            ...
                            @php
                                ...
                                foreach($section->content as $k => $item){
                                    if($k !== 'images' && $item !== 'icon' && $k !== 'select'){ $hasText = true; break; }
                                }
                            @endphp
                                @if($hasText)
                            <div class="col-12 mb-3">
                                <ul class="nav nav-tabs" role="tablist">
                                    ...
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link {{ $loop->first ? 'active' : '' }}"
                                            data-bs-toggle="tab"
                                            data-bs-target="#lang-tab-{{ $lang->code }}"
                                ...
                                <div class="tab-content border border-top-0 p-3 rounded-bottom">
                                    ...
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                         id="lang-tab-{{ $lang->code }}" role="tabpanel">
                                        <div class="row">
                                        @foreach ($section->content as $k => $item)
                                            @if($k !== 'images' && $item !== 'icon' && $k !== 'select')
                                                @php $fieldName = $k . '_' . $lang->code; @endphp
                                                @if($item === 'textarea')
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }} <span class="badge bg-secondary">{{ strtoupper($lang->code) }}</span></label>
                                                        <textarea rows="6" class="form-control" name="{{ $fieldName }}">{{ $content?->data_values->$fieldName ?? ($lang->is_default ? ($content?->data_values->$k ?? '') : '') }}</textarea>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }} <span class="badge bg-secondary">{{ strtoupper($lang->code) }}</span></label>
                                                        <textarea rows="6" class="form-control trumEdit" name="{{ $fieldName }}">{{ $content?->data_values->$fieldName ?? ($lang->is_default ? ($content?->data_values->$k ?? '') : '') }}</textarea>
                                                    </div>
                                                </div>
                                                @else
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="{{ $fieldName }}"
                                                            value="{{ $content?->data_values->$fieldName ?? ($lang->is_default ? ($content?->data_values->$k ?? '') : '') }}" />
                                                    </div>
                                                </div>
                                                @endif
                                            @endif
                            ...
                            {{-- ===== Images / Icons / Select (language-neutral) ===== --}}
                            @foreach ($section->content as $k => $item)
                                @if ($k == 'images')
                                                        ...
                                                        <div class="avatar-edit">
                                                            ...
                                                            <small class="mt-2">
                                                                @if ($section->content->images->$imgKey->size)
                                                                    @lang('Recomended size:'): <b>{{ $section->content->images->$imgKey->size }}</b> @lang('px').
                                                                @endif
                                                            </small>
                                                        </div>
                                                    </div>
                                ...
                                @elseif($k !== 'images')
                                    ...
                                    @elseif($languages->count() <= 1)
                                        {{-- Single language: show plain text fields (no tabs) --}}
                                        @if($item === 'textarea')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>{{ __(keyToTitle($k)) }}</label>
                                                        ...
                                                        @php
                                                            ...
                                                            if (!empty($section->element->images)) {
                                                                foreach ($section->element->images as $imgKey => $imgs) {
                                                                    $images[] = getImage(
                                                                        'assets/images/frontend/' . $key . '/' . ($data?->data_values->$imgKey ?? ''),
                            ...
                            @foreach ($section->element as $k => $type)
                                @if ($k != 'modal')
                                    ...
                                    @elseif($type == 'textarea')
                                        <div class="form-group">
                                            <label>{{ __(keyToTitle($k)) }}</label>
                                            <textarea rows="4" class="form-control" name="{{ $k }}" @if(!str_contains($k, '_')) required @endif></textarea>
                                        </div>
                                    ...
                                    @else
                                        <div class="form-group">
                                            <label>{{ __(keyToTitle($k)) }}</label>
                                            <input type="text" class="form-control" name="{{ $k }}"
                                                @if(!str_contains($k, '_')) required @endif />
                                        </div>
                                    @endif
                                @endif
                            @endforeach
