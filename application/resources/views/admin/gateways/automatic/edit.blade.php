                    ...
                    <div class="card-body">
                        ...
                        @isset($gateway->currencies)
                            @foreach($gateway->currencies as $gatewayCurrency)
                                <div class="card border--primary {{$loop->index == 0 ? 'mt-5' : ''}} mb-4 gateway-currency-card" data-currency="{{ $gatewayCurrency->currency }}">
    ...
    <script>
        (function ($) {
            ...
            $('.newCurrencyBtn').on('click', function () {
                ...
                var existingCurrencyCard = $('.gateway-currency-card[data-currency="' + getCurrencySelected + '"]');
                if (existingCurrencyCard.length) {
                    $('html, body').animate({ scrollTop: existingCurrencyCard.offset().top - 20 }, 'slow');
                    existingCurrencyCard.addClass('border--success');
                    setTimeout(function () {
                        existingCurrencyCard.removeClass('border--success');
                    }, 2000);
                    return;
                }
