...
@section('panel')
@include('admin.components.tabs.employee')
<div class="row">
    <div class="col-lg-12">
                ...
                <div class="table-responsive--md  table-responsive admin-table-responsive">
                    <table class="table table--light style--two">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <a href="{{ route('admin.employees.detail', $user->id) }}">{{$user->fullname}}
                                        ({{$user->username}})</a>
                                </td>
                                <td>
                                    {{ $user->email }}
                                ...
                                <td>
                                    @if (Request::routeIs('admin.employees.kyc.pending'))
                                ...
                                @endif
                                    ...
                                    @if (!$deleteBlockReason)
                                        <button type="button" class="btn btn-sm btn--danger confirmationBtn"
                                            data-question="{{ __('Are you sure you want to delete this employee? This action cannot be undone.') }}"
                                            data-action="{{ route('admin.employees.delete', $user->id) }}"
                                            aria-label="{{ __('Delete Employee') }}">
                                    ...
                                    @endif
                                    <a title="@lang('User Profile')" href="{{ route('admin.employees.detail', $user->id) }}"
                                        class="btn btn-sm btn--primary">
...
<div class="d-flex flex-wrap justify-content-end gap-2">
    <a href="{{ route('admin.employees.create') }}" class="btn btn-outline--primary"><i class="las la-plus"></i>@lang('Add New Employee')</a>
    ...
    <script>
        (function ($) {
            ...
            $(document).on('click', '.userStatus', function () {
                ...
                if (status === 1) {
                    modal.find('.status-modal-title').text(@json(__('Ban Employee')));
                    ...
                    modal.find('.status-modal-title').text(@json(__('Unban Employee')));
